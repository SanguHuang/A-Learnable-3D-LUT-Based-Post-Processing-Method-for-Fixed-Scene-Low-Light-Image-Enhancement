...
def main():
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            w_cons_cbcr = 0.0
            loss_cons_cbcr_value = 0.0

            # Accumulate gradients of the arithmetic mean over all src images.
            # Backward is executed once per image with a 1/M scale, which is exactly
            # equivalent to constructing one large mean-loss graph, but uses less GPU memory.
            ...
            for src_cpu, ref_cpu, name in step_samples:
                ...
                loss_rgb_light_i = charbonnier(out - ref).mean()
                loss_rgb_mse_i = F.mse_loss(out, ref)
                ...
                data_loss_i = (
                    ...
                    + w_rgb_light * loss_rgb_light_i
                    + w_rgb_mse * loss_rgb_mse_i
                    + w_cbcr * loss_cbcr_i
                ...
                sums["rgb"] += float(loss_rgb_light_i.detach().item())
                sums["rgb_mse"] += float(loss_rgb_mse_i.detach().item())
            ...
            # LUT-only regularization is identical for every source. Adding it once after
            # the averaged data loss is mathematically identical to averaging total losses.
            loss_id = lut.identity_loss()
            regularization_loss = w_id * loss_id
            ...
            loss_rgb_light_value = sums["rgb"] * inv_m
            loss_rgb_mse_value = sums["rgb_mse"] * inv_m
            ...
            ebri = ema_bri.update(loss_y_value)
            ebri_stat = ema_bri_stat.update(loss_bri_stat_value)
            ergb = ema_rgb.update(loss_rgb_light_value)
            ergb_mse = ema_rgb_mse.update(loss_rgb_mse_value)
            ...
            if step % log_every == 0:
                ...
                logger.log(
                    ...
                    f"CbCr={loss_cbcr_value:.4f} "
                    f"Y={loss_y_value:.4f} BriStat={loss_bri_stat_value:.4f} RGB={loss_rgb_light_value:.4f} RGB_MSE={loss_rgb_mse_value:.6f} "
